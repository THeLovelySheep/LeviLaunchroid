...
namespace {
    ...
    bool ends_with_so(const std::string& filename) {
        ...
        if (filename.length() < suffix_len) return false;
    ...
    void initialize_context(JNIEnv* env) {
        ...
        if (!app_context) {
            //LOGE("Failed to get global context");
            return;
        }
        ...
        for (auto& c : ctx->package_name) c = tolower(c);

        //ctx->mods_dir = (fs::path(ctx->internal_storage_path) / "Android" / "data" / ctx->package_name / "mods");
        ctx->mods_dir = (fs::path(ctx->internal_storage_path) / "games" / "org.levimc" / "mods");
    ...
    bool is_mod_enabled(const fs::path& json_path, const std::string& mod_filename) {
    ...
    void load_mods(const fs::path& mods_dir, const fs::path& target_dir) {
        ...
        for (const auto& entry : fs::directory_iterator(mods_dir)) {
            if (entry.is_regular_file() && ends_with_so(entry.path().filename().string())) {
                fs::path json_path = fs::path(g_ctx->mods_dir) / "mods_config.json";

                auto dest_path = target_dir / entry.path().filename();
                if (!is_mod_enabled(json_path, entry.path().filename().c_str())) continue;
                ...
                if (auto handle = dlopen(dest_path.c_str(), RTLD_NOW)) {
                    //LOG("Loaded mod: %s", entry.path().filename().c_str());
                }
                else {
                    //LOGE("Failed to load %s: %s", entry.path().filename().c_str(), dlerror());
                }
            }
        }
    }
